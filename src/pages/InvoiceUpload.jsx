...
export default function InvoiceUpload() {
  ...
  function handleFile(f) {
    ...
    if (f.size > MAX_SIZE_MB * 1024 * 1024) {
      setError(`حجم الملف كبير — الحد الأقصى ${MAX_SIZE_MB} ميغابايت`)
      return
    }
    setFile(f)
    setDone(false)
  ...
  return (
    <div className="max-w-xl mx-auto space-y-5">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">ارفع الفاتورة أو الإيصال — سيراجعه المحاسب ويعتمده</p>
      </div>
      ...
      {!done && file && (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <button onClick={reset} className="text-slate-400 hover:text-red-500 text-xl transition-colors">✕</button>
          </div>
          {preview && (
            <img src={preview} alt="preview" className="w-full max-h-60 object-contain rounded-xl bg-slate-50"/>
          )}
