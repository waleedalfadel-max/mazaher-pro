...
async function financeApi(operation, payload = {}, file = null) {
  const messages = {
    ...
    CONFLICT: 'تغيّرت البيانات بواسطة مستخدم آخر. أغلق المراجعة وافتحها مجددًا قبل الحفظ أو الاعتماد',
    FILE_REQUIRED: 'اختر ملفًا لا يتجاوز حجمه 5 ميجابايت', FILE_TYPE: 'اختر ملف PDF أو JPG أو PNG',
    FILE_UNAVAILABLE: 'تعذر الوصول إلى الملف. حاول مرة أخرى', REQUEST_TOO_LARGE: 'حجم الطلب أكبر من الحد المسموح',
    REQUEST_ID_REUSED: 'تغير محتوى طلب محفوظ. حدّث الصفحة قبل المتابعة', NOT_FOUND: 'المستند غير متاح لك',
  };
  const token = employeeToken() || (await auth.auth.getSession()).data.session?.access_token;
  const request = { operation, tenant, ...payload }, headers = { apikey: key, ...(token ? { Authorization: `Bearer ${token}` } : {}) };
  let body;
  if (file) { body = new FormData(); body.set('request', JSON.stringify(request)); body.set('file', file); }
  else { headers['Content-Type'] = 'application/json'; body = JSON.stringify(request); }
  let response;
  try { response = await fetch(`${url}/functions/v1/tatmira-finance`, { method: 'POST', headers, body }); }
  catch { const error = new Error(messages.FINANCE_UNAVAILABLE); error.code = 'FINANCE_UNAVAILABLE'; throw error; }
  if (response.ok && operation === 'file') return response.blob();
  const result = await response.json();
  if (!response.ok) { const error = new Error(messages[result.error] || ERRORS[result.error] || result.message || 'تعذر إكمال العملية'); error.code = result.error; throw error; }
  return result;
}
...
function App() {
  ...
  return (
    <div className="container">
      ...
      <Workspace key={identity.actor.id} actor={identity.actor} api={financeApi} employees={<Employees />}
        onExpired={() => { generation.current++; sessionStorage.removeItem(TOKEN_KEY); setIdentity(null); setError(ERRORS.INVALID_SESSION); }} />
    </div>
  );
}
